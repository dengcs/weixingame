...
package com.hsharma.hungryHero.screens
{
	...
	public class InGame extends Sprite
	{
		...
		{
			itemsPool = new PoolItem(this,foodItemCreate, foodItemClean);
		}
		...
		private function foodItemCreate():Item
		{
			...
			foodItem.x = GameConstants.stageWidth + 10000//foodItem.width * 2;
		...
		{
			obstaclesPool = new PoolObstacle(this,obstacleCreate, obstacleClean, 4, 10);
		}
		...
		{
			eatParticlesPool = new PoolParticle(this,eatParticleCreate, eatParticleClean, 20, 30);
		}
		...
		{
			windParticlesPool = new PoolParticle(this,windParticleCreate, windParticleClean, 10, 30);
		}
		...
		private function launchHero():void
		{
			...
			// Touch interaction
			stage.on(Event.MOUSE_MOVE,this, onTouch);
			
			// Game tick
			Laya.timer.frameLoop(1,this, onGameTick);
		}
		...
		private function onTouch(event:Event):void
		{
			...
			touchX = mouseX;//event.stageX; //touch.globalX;
			touchY = mouseY//event.stageY;// touch.globalY;
		}
			...
			if (!gamePaused)
			{
				...
				switch(gameState)
				{
					...
					case  com.hsharma.hungryHero.GameConstants.GAME_STATE_IDLE:
						...
						if (hero.y < gameArea.y + hero.height * 0.5)    
						{
							hero.y = gameArea.y + hero.height * 0.5;
							hero.rotation = deg2rad(0);
						}
						break;
					...
					case com.hsharma.hungryHero.GameConstants.GAME_STATE_FLYING:
						...
						else
						{
							...
							if (coffee <= 0)
							{
								...
								hero.y -= (hero.y - (gameArea.y + gameArea.height)/2) * 0.1;
			...
			switch (pattern)
			{
				case 1:
					...
					if (Math.random() > 0.9)
					{
						...
						patternPosY = Math.floor(Math.random() * (gameArea.bottom - gameArea.y + 1)) + gameArea.y;
					}
				...
				case 3:
					...
					else if ( patternDirection == -1 && patternPosY < gameArea.y )
					{
						patternDirection = 1;
					}
					
					if (patternPosY >= gameArea.y && patternPosY <= gameArea.bottom)
					{
						// Checkout item from pool and set the type of item.
						itemToTrack = itemsPool.checkOut();
						itemToTrack.foodItemType = Math.ceil(Math.random() * 5);
		...
		private function disposeItemTemporarily(animateId:uint, item:Item):void
		{
			...
			item.x = GameConstants.stageWidth + 10000 * 2;
		...
		private function createObstacle(_type:int = 1, _distance:Number = 0):void
		{
			...
			else
			{
				...
				obstacle.y = Math.floor(Math.random() * (gameArea.bottom-obstacle.height - gameArea.y + 1)) + gameArea.y;
		...
		private function shakeAnimation(event:Event):void
		{
			...
			if (cameraShake > 0)
			{
				...
				this.x = Math.floor(Math.random() * cameraShake - cameraShake * 0.5); 
				// Shake up down randomly.
				this.y = Math.floor(Math.random() * cameraShake - cameraShake * 0.5); 
			}
			else if (x != 0) 
			{
